...
def get_TransformedData(data):
    ...
    df4= pd.DataFrame(np.zeros((l,5)),index=z,columns=['release_date','avg_rating','gender_preference','occupation','age'])
   ...
   # df4['occupation'] = data['occupation'].map({'retired':'R', 'salesman':'S', 'engineer':'E', 'librarian':'LIB', 'student':'S', 'other':'O',
    #   'executive':'EXE', 'administrator':'AD', 'artist':'A', 'entertainment':'ENT', 'educator':'EDU',
     #  'scientist':'SCNT', 'none':'NA', 'programmer':'PRG', 'homemaker':'HM', 'marketing':'MKT',
     #  'technician':'TEC', 'lawyer':'LAW', 'writer':'WRT', 'healthcare':'HC', 'doctor':'DOC'})
    #df4['age']=pd.cut(data['age'],bins=[7,17,27,37,47,57,67,77],labels=['A1','A2','A3','A4','A5','A6','A7'])
